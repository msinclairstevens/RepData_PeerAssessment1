...
```r
table(calcsfordate$totalfordate) #53 days.
```
...
```r
min(calcsfordate$totalfordate, na.rm=TRUE)
```
...
```r
max(calcsfordate$totalfordate, na.rm=TRUE)
```
...
```r
mean(calcsfordate$totalfordate, na.rm=TRUE)
```
...
```r
as.double(median(calcsfordate$totalfordate, na.rm=TRUE))
```
...
For me challenge to this histogram is deciding what a meaningful number of breakpoints should be. 
...
I couldn't tell what this question wanted so I answered it two ways. 
1. I took the mean and median of the 53 totals used for the histogram. This is so I could see how many daily totals were above and below average and get a sense of any skew.  
2. I calculated the mean and median for the total number of steps taken each day. As each day has 288 observations and the majority of them are zero (think eight hours of sleep and eight hours of sitting at work), each daily graph of steps is very skewed and the median is always zero.
...
```r
min(calcsforinterval$totalforinterval, na.rm=TRUE)
```
...
```r
max(calcsforinterval$totalforinterval, na.rm=TRUE)
```
...
```r
mean(calcsforinterval$totalforinterval, na.rm=TRUE)
```
...
```r
as.double(median(calcsforinterval$totalforinterval, na.rm=TRUE))
```
...
Eight days (columns) have NAs so ALL the rows have NAs. Isn't it more interesting to know which of the columns(dates) have NAs?
...
If I had more time, I might try it both ways to see if it makes any difference. Next time.
